...
float* compute_all_pairs_opinion_differences(float* opinions,int num_nodes){
    float *differences = malloc(sizeof(float)*num_nodes*num_nodes);
    if(!differences) return NULL;
    for (int i = 0; i < num_nodes; i++) {
        for (int j = 0; j < num_nodes; j++) {
            differences[i * num_nodes + j] = fabsf(opinions[i] - opinions[j]);
        }
    }
    return differences;
}
...
float return_max(float* matrix, size_t size) {
    if (!matrix) return -INFINITY;  // or some error value
...
void apply_natural_decay(graph* topology, float decay_rate, float min_bond_strength) {
    int num_nodes = topology->num_nodes;
    for (int i = 0; i < num_nodes; i++) {
        for (int j = 0; j < num_nodes; j++) {
            if (i == j) continue;
            if (!topology->edges[i * num_nodes + j]) continue;
            float edge_weight = topology->edge_weights[i * num_nodes + j];
            ...
            if (edge_weight < 0.0f) edge_weight = 0.0f;
            if (edge_weight > 1.0f) edge_weight = 1.0f;
            topology->edge_weights[i * num_nodes + j] = edge_weight;
            if (topology->is_directed)
                topology->edge_weights[i * num_nodes + j] = topology->edge_weights[j * num_nodes + i];
        }
    }
}
    ...
    const float minimum_bond_strength) 
{
    ...
    for (int node_i = 0; node_i < num_nodes; node_i++) {
        for (int node_j = 0; node_j < num_nodes; node_j++) {
            ...
            if (!topology->edges[node_i * num_nodes + node_j]) continue;

            float opinion_difference = fabsf(opinions[node_i] - opinions[node_j]);
            float edge_weight = topology->edge_weights[node_i * num_nodes + node_j];
            float bond_strength = 1.0f - edge_weight;

            // Reinforce bond if opinions are similar
            if (opinion_difference < opinion_similarity_threshold) {
                float reinforcement_amount = bond_reinforcement_rate * (1.0f - opinion_difference) * (1.0f - bond_strength);
                bond_strength += reinforcement_amount;
            }
            else {
                float weakening_amount = bond_weakening_rate * opinion_difference * bond_strength;
                bond_strength -= weakening_amount;
            }

            // Clamp bond strength and remove edge if below threshold
            if (bond_strength > 1.0f) bond_strength = 1.0f;
            ...
            topology->edge_weights[node_i * num_nodes + node_j] = edge_weight;

            if (topology->is_directed) {
                topology->edge_weights[node_j * num_nodes + node_i] = edge_weight;
            }
        }
    }
}
...
void create_edges_by_distance_and_opinion_similarity(
    graph* topology,
    float* opinions,
    float* distances,               // shortest path distances, INF if no path
    ...
    int num_nodes = topology->num_nodes;
    float* differences = compute_all_pairs_opinion_differences(opinions, num_nodes);
    if (!differences) return;

    float max_opinion_diff = return_max(differences, num_nodes * num_nodes);
    if (max_opinion_diff == 0) max_opinion_diff = 1.0f; // avoid div by zero

    for (int i = 0; i < num_nodes; i++) {
        for (int j = 0; j < num_nodes; j++) {
            if (i == j) continue;
            if (topology->edges[i * num_nodes + j]) continue;  // Skip existing edges

            float dist = distances[i * num_nodes + j];
            float opinion_diff = differences[i * num_nodes + j];
            float sigma = 0.2f;  // tune this parameter to control width of similarity decay
            float opinion_similarity = expf(- (opinion_diff * opinion_diff) / (sigma * sigma));
            ...
            if (!isinf(dist)) {
                ...
                creation_prob += base_creation_probability * distance_factor_scale * distance_factor * similarity_factor * opinion_similarity;
            }
            ...
            if (rand_val < creation_prob) {
                ...
                topology->edge_weights[i * num_nodes + j] = initial_bond_strength;

                if (!topology->is_directed) {
                    topology->edges[j * num_nodes + i] = 1;
                    topology->edge_weights[j * num_nodes + i] = initial_bond_strength;
                }
...
void social_impact_async_mult_update_temporal_topology(opinion_model* model)
{
    ...
    update_topology_mixed(model->network,
              opinions,
              dist,
              0.6f,    // opinion_similarity_threshold
