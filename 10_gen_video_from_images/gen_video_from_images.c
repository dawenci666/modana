				...
				const char *video_filename, int fps)
{
	...
	snprintf(images_path, sizeof(images_path), "%s/images",
		 directoryname);
	...
	if (!video_filename) {
		snprintf(full_output_path, sizeof(full_output_path),
			 "%s/animation.mp4", directoryname);
	} else {
		snprintf(full_output_path, sizeof(full_output_path), "%s",
			 video_filename);
	}

	// Create temporary file list for ffmpeg
	char list_file[PATH_MAX];
	snprintf(list_file, sizeof(list_file), "%s/imagelist.txt",
		 directoryname);
