...
// helper: intersect two lines (p1→p2 shifted outward) and (q1→q2 shifted outward)
static point_t line_intersect(point_t p1, point_t p2, float pad,
                              point_t q1, point_t q2, float qpad) {
    ...
    // Shifted line points
    float sx1 = p1.x + nx1 * pad, sy1 = p1.y + ny1 * pad;
    float sx2 = p2.x + nx1 * pad, sy2 = p2.y + ny1 * pad;
    float tx1 = q1.x + nx2 * qpad, ty1 = q1.y + ny2 * qpad;
    float tx2 = q2.x + nx2 * qpad, ty2 = q2.y + ny2 * qpad;

    // Solve intersection of (sx1→sx2) and (tx1→tx2)
    float A1 = sy2 - sy1, B1 = sx1 - sx2, C1 = A1*sx1 + B1*sy1;
    float A2 = ty2 - ty1, B2 = tx1 - tx2, C2 = A2*tx1 + B2*ty1;
    float det = A1*B2 - A2*B1;
    if (fabsf(det) < 1e-6f) {
        // nearly parallel: fallback to midpoint
        return (point_t){ .x = (sx2+tx1)*0.5f, .y = (sy2+ty1)*0.5f };
    }
    return (point_t){
        .x = (B2*C1 - B1*C2)/det,
        .y = (A1*C2 - A2*C1)/det
...
void draw_convex_hull(cairo_t* cr, float* positions, int* cluster_map, int cluster_id, int n) {
    // 1) collect center points
    int m=0;
    for(int i=0;i<n;i++) if(cluster_map[i]==cluster_id) m++;
    if(m<3) return;
    point_t* pts = malloc(m*sizeof(point_t));
    for(int i=0, j=0;i<n;i++){
        if(cluster_map[i]==cluster_id){
            pts[j].x = positions[2*i];
            pts[j].y = positions[2*i+1];
            j++;
        }
    }
    // 2) get hull indices (0..m-1)
    int* hull_idx = malloc(m*sizeof(int));
    int hsz = convex_hull(pts, m, hull_idx);
    if(hsz<3){ free(pts); free(hull_idx); return; }

    // 3) build offset polygon by intersecting shifted edge‐lines
    float pad = NODE_RADIUS_MAX * 1.3f;
    point_t* off = malloc(hsz*sizeof(point_t));
    for(int i=0;i<hsz;i++){
        point_t A = pts[hull_idx[i]];
        point_t B = pts[hull_idx[(i+1)%hsz]];
        point_t C = pts[hull_idx[(i+2)%hsz]];
        // intersect shifted line AB with shifted line BC
        off[i] = line_intersect(A,B,pad,  B,C,pad);
    }

    // 4) draw smooth blob
    cairo_set_source_rgba(cr, 0.2,0.6,0.8,0.35);
    cairo_set_line_width(cr, 4.0);
    draw_smooth_hull(cr, off, hsz);
