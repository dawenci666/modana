...
void draw_edge(cairo_t* cr, float x1, float y1, float x2, float y2, float weight) {
    // Line width inversely proportional to edge weight
    float width = 10.0f * (1.0f - weight);
    ...
    float dx = x2 - x1;
    float dy = y2 - y1;
    // Normal for curvature
    float nx = -dy;
    float ny = dx;
    float nlen = sqrtf(nx*nx + ny*ny);
    if (nlen < 1e-6f) nlen = 1.0f;
    nx /= nlen; ny /= nlen;
    // Offset control point by weight‑scaled normal
    float offset = 100.0f;
    float cx = mx + nx * offset * (1.0f - weight);
    float cy = my + ny * offset * (1.0f - weight);

    cairo_move_to(cr, x1, y1);
    cairo_curve_to(cr,
                   (x1 + cx) * 0.5f, (y1 + cy) * 0.5f,
                   (x2 + cx) * 0.5f, (y2 + cy) * 0.5f,
                   x2, y2);
    cairo_stroke(cr);
}
...
void draw_convex_hull(cairo_t* cr, float* positions, int* cluster_map, int cluster_id, int n) {
    // 1) Collect cluster members, sampling circle boundaries
    int nodeCount = 0;
    for (int i = 0; i < n; i++)
        if (cluster_map[i] == cluster_id) nodeCount++;
    if (nodeCount < 1) return;

    // 5 samples per node: center + 4 cardinal boundary points
    int sampleCount = nodeCount * 5;
    point_t* points = malloc(sampleCount * sizeof(point_t));
    int idx = 0;
    float R = NODE_RADIUS_MAX * 0.75f;  // match your draw_node radius

    for (int i = 0; i < n; i++) {
        if (cluster_map[i] != cluster_id) continue;
        float cx = positions[2*i];
        float cy = positions[2*i+1];
        // center
        points[idx++] = (point_t){ .x = cx,     .y = cy,     .idx = i };
        // east
        points[idx++] = (point_t){ .x = cx+R,   .y = cy,     .idx = i };
        // west
        points[idx++] = (point_t){ .x = cx-R,   .y = cy,     .idx = i };
        // south
        points[idx++] = (point_t){ .x = cx,     .y = cy+R,   .idx = i };
        // north
        points[idx++] = (point_t){ .x = cx,     .y = cy-R,   .idx = i };
    }

    // 2) Compute convex hull on the expanded set
    int* hull_indices = malloc(sampleCount * sizeof(int));
    int hull_size = convex_hull(points, sampleCount, hull_indices);
    if (hull_size < 3) {
        free(points);
        free(hull_indices);
        return;
    ...
    // 3) Extract hull points into local array
    point_t* hull_pts = malloc(hull_size * sizeof(point_t));
    for (int h = 0; h < hull_size; ++h) {
        int sampIdx = hull_indices[h];
        hull_pts[h] = points[sampIdx];
    }

    // 4) Pad outward by averaging normals at each hull vertex
    float padding = NODE_RADIUS_MAX * 1.3f;
    for (int i = 0; i < hull_size; ++i) {
        // previous/next with wrap
        point_t P = hull_pts[i];
        point_t Pprev = hull_pts[(i-1 + hull_size) % hull_size];
        point_t Pnext = hull_pts[(i+1) % hull_size];

        // edge normals
        float ex = Pnext.x - P.x, ey = Pnext.y - P.y;
        float elen = sqrtf(ex*ex + ey*ey);
        if (elen < 1e-6f) elen = 1;
        ex /= elen; ey /= elen;
        float nx1 = -ey, ny1 = ex;

        float px = P.x - Pprev.x, py = P.y - Pprev.y;
        ...
        // average normal
        float ax = (nx1 + nx2)*0.5f, ay = (ny1 + ny2)*0.5f;
        float alen = sqrtf(ax*ax + ay*ay);
        if (alen < 1e-6f) alen = 1;
        ax /= alen; ay /= alen;

        // apply padding
        hull_pts[i].x += ax * padding;
        hull_pts[i].y += ay * padding;
    }

    // 5) Draw smooth, translucent blob
    cairo_set_source_rgba(cr, 0.2, 0.6, 0.8, 0.35);
    cairo_set_line_width(cr, 4.0);
    draw_smooth_hull(cr, hull_pts, hull_size);
