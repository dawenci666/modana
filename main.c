...
void consensus_time_vs_nodes(int runs) {
    ...
    char base_dir[256];
    sprintf(base_dir, "./simls_raw_data/consensus_vs_nodes-%s", timestamp);
    ...
    for (int idx = 0; idx < max_points; idx++) {
        ...
        for (int run = 0; run < runs; run++) {
            ...
            if (er_time < 999) {
                ...
                counted_runs_er++;
                if ((float)er_time > er_max) er_max = (float)er_time;
                if ((float)er_time < er_min) er_min = (float)er_time;
            }else{
            ...
            // WS graph
            graph* ws = generate_watts_strogatz(nnodes, 4, 0.1f, 0);
            char outdir_ws[512];
            sprintf(outdir_ws, "%s/ws=0.1,%d,0_sim=ws,2,1_run%d", base_dir, nnodes, run);
            ...
            if (ws_time < 999) {
                ...
                counted_runs_ws++;
                if ((float)ws_time > ws_max) ws_max = (float)ws_time;
                if ((float)ws_time < ws_min) ws_min = (float)ws_time;
            }
            ...
            char outdir_ba[512];
            sprintf(outdir_ba, "%s/ba=2,%d,0_sim=ba,2,1_run%d", base_dir, nnodes, run);
            ...
            if (ba_time < 999) {
                ...
                counted_runs_ba++;
                if ((float)ba_time > ba_max) ba_max = (float)ba_time;
                if ((float)ba_time < ba_min) ba_min = (float)ba_time;
            }
        ...
        if(er_sum==0){
            printf("nnodes=%d, er didnt converge for any of the 50 runs\n", nnodes);
        }
        if(ws_sum==0){
            printf("nnodes=%d, ws didnt converge for any of the 50 runs\n", nnodes);
        }
        if(ba_sum==0){
            printf("nnodes=%d, ba didnt converge for any of the 50 runs\n", nnodes);
        }
    ...
    fprintf(f_combined, "Nodes,ER_mean,ER_error,WS_mean,WS_error,BA_mean,BA_error\n");
