...
static bool is_valid_opinion(float *opinions, int *cluster, int size,
			     int node, float max_op_diff)
{
	for (int i = 0; i < size; i++) {
		if (fabsf(opinions[cluster[i]] - opinions[node]) >
		    max_op_diff)
			return false;
	}
	return true;
...
cluster_result find_disjoint_maximal_opinion_clusters(graph *g,
						      float *opinions,
						      float max_distance,
						      float max_op_diff)
{
	...
	for (int start = 0; start < n; start++) {
		...
		while (front < rear) {
			...
			for (int neighbor = 0; neighbor < n; neighbor++) {
				...
				if (d <= max_distance
				    && is_connected(g, curr, neighbor)
				    && is_valid_opinion(opinions, cluster,
							cluster_size,
							neighbor,
							max_op_diff)) {
					queue[rear++] = neighbor;
		...
		float avg_op =
		    (cluster_size > 0) ? sum_op / cluster_size : 0.0f;
...
cluster_result find_overlapping_maximal_opinion_clusters(graph *g,
							 float *opinions,
							 float
							 max_distance,
							 float max_op_diff)
{
	...
	for (int start = 0; start < n; start++) {
		...
		if (cluster_size > 1) {
			...
			float avg_op =
			    (cluster_size >
			     0) ? sum_op / cluster_size : 0.0f;

			result.clusters[result.num_clusters] = cluster;
			result.cluster_sizes[result.num_clusters] =
			    cluster_size;
			result.avg_opinions[result.num_clusters] = avg_op;	// store avg opinion
			result.num_clusters++;
		} else {
...
char **build_opinion_labels(const float *opinions, size_t num_nodes)
{
	...
	for (size_t i = 0; i < num_nodes; i++) {
		...
		snprintf(labels[i], MAX_LABEL_LEN, "%zu - %.3f", i,
			 opinions[i]);
	}
	return labels;
}
...
float *read_opinions(const char *filename, size_t *out_num_nodes)
{
	...
	while (fscanf(file, "%zu %f\n", &idx, &val) == 2) {
		if (idx >= count) {
			fprintf(stderr,
				"Warning: node index %zu out of range in opinions file\n",
				idx);
			continue;
		}
		opinions[idx] = val;
						    ...
						    int num_nodes_hint)
{
	...
	RGBColor *edge_colors =
	    malloc(sizeof(RGBColor) * num_nodes * num_nodes);
	...
	for (size_t i = 0; i < num_nodes; i++) {
		for (size_t j = 0; j < num_nodes; j++) {
			...
			edge_colors[i * num_nodes + j] = (RGBColor) {
			r, g_col, b};
		}
	}
	...
	cluster_result clusters =
	    find_overlapping_maximal_opinion_clusters(g, opinions, 3, 0.5);
