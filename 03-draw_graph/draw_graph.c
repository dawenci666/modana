...
void draw_cluster_hulls(cairo_t *cr, VisNode* layout, float* node_sizes,
    ...
    if (!clusters || clusters->num_clusters == 0) return;
    ...
    for (int c = 0; c < clusters->num_clusters; c++) {
        ...
        for (int i = 0; i < cluster_size; i++) {
            ...
            for (int p = 0; p < POINTS_PER_NODE; p++) {
                double angle = 2.0 * M_PI * p / POINTS_PER_NODE;
        ...
        int hull_size = 0;
        VisNode* hull = compute_convex_hull(points, total_points, &hull_size);
        free(points);
...
int position_ok(VisNode *nodes, float* node_sizes, float curr_radius, int placed_nodes, double x, double y)
{
...
void save_graph_as_image(graph *g, RGBColor* node_colors, RGBColor* edge_colors,
                         float* node_sizes, const char *filename, char **labels, VisNode *layout, cluster_result* clusters)
{
    ...
    for (size_t i = 0; i < n; i++) {
        for (size_t j = i + 1; j < n; j++) {
            ...
            if (g->edges[edge_idx] != 0) {
                ...
                float g_col = edge_colors[edge_idx].g / 255.0f;
                float b = edge_colors[edge_idx].b / 255.0f;

                float bond_strength = 1.0f - g->edge_weights[edge_idx];
                float edge_width = 5.0f + bond_strength * (10.0f - 5.0f);
    ...
    // Draw nodes
    for (size_t i = 0; i < n; i++) {
        float r = 128.0f / 255.0f, g_col = 128.0f / 255.0f, b = 128.0f / 255.0f;
        if (node_colors) {
        ...
        cairo_set_source_rgb(cr, r, g_col, b);
        cairo_arc(cr, layout[i].x, layout[i].y, node_sizes[i], 0, 2 * M_PI);
        cairo_fill(cr);
