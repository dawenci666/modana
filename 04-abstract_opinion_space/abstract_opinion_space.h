...
typedef struct opinion_space {
    ...
    const void* (*get_opinion)(const struct opinion_space* os, int index);
    
    // Setter method
    bool (*set_opinion)(struct opinion_space* os, int index, const void* value);
...
opinion_space* create_opinion_space(size_t num_agents, size_t element_size);

opinion_space* create_finite_domain_space(
    size_t num_agents,
    const void* domain_samples,
    size_t element_size,
    void (*sampler)(struct opinion_space* os, int target_idx, const void* domain)
);
