...
void write_current_state(opinion_model *model, size_t current_step, const char *directoryname)
{
    if (!directoryname) {
    ...
    // Write opinions
    snprintf(filepath, sizeof(filepath), "%s/%zu.opinions", directoryname, current_step);
    ...
    // Write graph
    snprintf(filepath, sizeof(filepath), "%s/%zu.graph", directoryname, current_step);
...
int run_simulation(opinion_model *model, size_t max_steps, float convergence_threshold, const char* directoryname, int save_data)
{
    ...
    if (!model->opinion_space->opinions) {
        fprintf(stderr, "model->opinion_space->opinions is NULL\n");
        return -1;
    }
    ...
    for (size_t step = 0; step < max_steps; step++) {
        ...
        for (size_t i = 0; i < n; i++) {
            float *current_op = (float *)((char *)opinions + i * esize);
